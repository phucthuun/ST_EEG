...
# EEG ----
eeg_combined_df <- load_eegdata(vps, file.path(pathlist$eeg_data_path, "Revise_FullPhase"))
...
# Check: Are Beh Duration equal to EEG move duration?
ggplot(rawData) +
  geom_point(aes(x = DURATION, y = MovDur), alpha = .1) +
  labs(
    x = "Move duration in behavioral data (s)",
    y = "Move duration in EEG data (s)"
  ) +
  coord_fixed(ratio = 1)
...
predictData <- parData %>%
  filter(unimodal == "Self-touch" & gain != "1.Unity" & timing %in% c("Full Movement")) %>%
...
plotData <- parData %>% filter(unimodal == "Self-touch" & gain == "2.NonUnity")
...
ggplot(
  ...
  geom_smooth(method = "lm", show.legend = T) +
  labs(
    title = "Predicting Judgement Error with Difference of Beta Power (Unity conditions)",
    x = "Beta Power Difference: C3 - C4",
    y = "ABSOLUTE Judgement Error"
  ) +
  scale_color_manual("Task", values = palette$two.color1) +
