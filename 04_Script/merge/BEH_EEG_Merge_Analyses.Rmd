...
p1 = ggplot(data = plotData %>% filter(unimodal == "Self-touch")) +
  ...
  geom_ribbon(aes(x = Target_length, ymin = mean - se, ymax = mean + se,
                  fill = gain, group = gain), alpha = 0.15, show.legend = F) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +
  facet_grid(task ~ mov) +
  scale_x_continuous(limits = c(2, 10), breaks = seq(4, 8, by = 2)) +
  scale_y_continuous(limits = c(2, 10), breaks = seq(2, 10, by = 2)) +
  labs(title = "Self-touch Conditions",
...
p2 = ggplot(data = plotData %>% filter(unimodal == "Unimodal")) +
  ...
  geom_ribbon(aes(x = Target_length, ymin = mean - se, ymax = mean + se,
                  fill = gain, group = gain), alpha = 0.15, show.legend = T) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +
  facet_grid(task ~ mov, drop = T) +
  scale_x_continuous(limits = c(2, 10), breaks = seq(4, 8, by = 2)) +
  scale_y_continuous(limits = c(2, 10), breaks = seq(2, 10, by = 2)) +
  labs(title = "Unimodal Conditions",
...
#### Signed Error as Dependent Variable

This is the wrong visualization of the right model
...
predictData <- parData %>%
  ...
  mutate( estError = (resp - Target_length) / Target_length)
...
plotData <- parData %>% filter(unimodal == "Self-touch" & flipped.gain != "1.Unity" & timing == "Full Movement")
...
ggplot(
  ...
  labs(
    title = "Predicting Judgement Error with Difference of Beta Power (A/P Conditions)",
    subtitle = "PowerDiff x Task x Movement type x Gain",
...
#### Different computation of Signed Error as Dependent Variable

Model 0: Does interhemispheric balance predict How accurate participants are around the target?

```{r fig.width=16, fig.height=10, echo=TRUE, warning=FALSE, message=FALSE} 
filter.timing="Full"
predictData <- parData %>%
  filter(flipped.gain != "1.Unity" & unimodal == "Self-touch" & timing == paste(filter.timing,"Movement")) %>%
  mutate(sbjN = factor(sbjN))%>%
...
xlsx::write.xlsx(x=summary_df, sheetName="p0", file=file.path(pathlist$result_path,"lmer_results.xlsx"), append=TRUE)
...
p0 = ggplot(pred, aes(x = x, y = predicted, color = group)) +
  ...
  labs(
    # title = paste("Predicting Judgement Error with Beta Power",filter.timing,"Self-touch"),
    subtitle = "y(-1=underestimation, 0=accurate, +1=overestimation)",
    x = "Beta Power Difference: C3 - C4",
    y = "Judgement Error"
  )+
  scale_color_manual("Task", values = palette$two.color1) +
...
Model 1: Does interhemispheric balance predict if participants lean towards touch or move signal in each condition?
...
predictData <- parData %>%
  filter(flipped.gain != "1.Unity" & unimodal == "Self-touch" & timing == paste(filter.timing,"Movement")) %>%
  mutate(sbjN = factor(sbjN))%>%
  mutate(estError=(resp-touchLength)/(movLength-touchLength))
...
summary_df <- as.data.frame(summary(model)$coefficients) %>% 
 mutate_if(is.numeric, round,digits=2)%>%
  rownames_to_column("factor")
xlsx::write.xlsx(x=summary_df, sheetName="p1", file=file.path(pathlist$result_path,"lmer_results.xlsx"), append=TRUE)
...
p1 = ggplot(pred, aes(x = x, y = predicted, color = group)) +
  geom_hline(yintercept = 0, linetype = "solid", color = palette$two.color1[2]) +
  geom_hline(yintercept = 1, linetype = "solid", color = palette$two.color1[1]) +
  ...
  labs(
    # title = paste("Predicting Judgement Error with Beta Power",filter.timing,"Self-touch"),
    subtitle = "y(0=toward movement, 1=toward touch)",
    x = "Beta Power Difference: C3 - C4",
    y = "Judgement Error"
  )+
  scale_color_manual("Task", values = palette$two.color1) +
...
Model 2: Does the degree of interference depend on interhemispheric balance and which modality is magnified?
...
predictData <- parData %>%
  filter(flipped.gain != "1.Unity" & unimodal == "Self-touch" & timing == paste(filter.timing,"Movement")) %>%
  mutate(sbjN = factor(sbjN))%>%
  mutate(estError=(resp-Target_length)/(Interference_length-Target_length))
...
summary_df <- as.data.frame(summary(model)$coefficients) %>% 
 mutate_if(is.numeric, round,digits=2)%>%
  rownames_to_column("factor")
xlsx::write.xlsx(x=summary_df, sheetName="p2", file=file.path(pathlist$result_path,"lmer_results.xlsx"), append=TRUE)

# Generate predictions for PowerDiff across task, mov, and gain
pred <- ggpredict(model, terms = c("PowerDiff [all]", "task", "mov", "gain"))

# Plot predictions
p2=ggplot(pred, aes(x = x, y = predicted, color = group)) +
  geom_hline(yintercept = c(0,1), linetype = "solid") +
  ...
  labs(
    # title = paste("Predicting Judgement Error with Beta Power",filter.timing,"Self-touch"),
    subtitle = "y(0=toward target, 1=toward interference)",
    x = "Beta Power Difference: C3 - C4",
    y = "Judgement Error",
    color = "Task",
  )+
...
Model 3: Does the degree of interference depend on interhemispheric balance and whether the judged modality is smaller or larger than the interfering modality?
```{r fig.width=16, fig.height=10, echo=TRUE, warning=FALSE, message=FALSE} 
filter.timing="Full"
predictData <- parData %>%
  filter(flipped.gain != "1.Unity" & unimodal == "Self-touch" & timing == paste(filter.timing,"Movement")) %>%
...
xlsx::write.xlsx(x=summary_df, sheetName="p3", file=file.path(pathlist$result_path,"lmer_results.xlsx"), append=TRUE)

# Generate predictions for PowerDiff across task, mov, and gain
pred <- ggpredict(model, terms = c("PowerDiff [all]", "task", "mov", "GAIN"))

# Plot predictions
p3=ggplot(pred, aes(x = x, y = predicted, color = group)) +
  geom_hline(yintercept = c(0,1), linetype = "solid") +
  geom_hline(yintercept = 0.5, linetype = "dashed") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  annotate("text", x = -.5, y = +Inf, label = sprintf("\u2190\u2190\u2190\nmotor-focused"), color = palette$two.color1[2], hjust = 1, vjust = "top", size = 12 / .pt) +
