...
# Plot
p1 = ggplot(data = plotData_subject %>% filter(unimodal == "Self-touch")) +
...
p2 = ggplot(data = plotData_subject %>% filter(unimodal == "Unimodal")) +
...
#### Absolute Error as Dependent Variable
...
ggplot(
  ...
  labs(
    ...
    subtitle = "PowerDiff x Task x Movement type",
...
plotData <- parData %>% filter(unimodal == "Self-touch" & flipped.gain == "2.NonUnity")
...
plotData <- parData %>% filter(unimodal == "Self-touch" & flipped.gain == "2.NonUnity" & mov == "Active")
...
plotData <- parData %>% filter(unimodal == "Self-touch" & flipped.gain == "2.NonUnity" & mov == "Passive")
...
Modelling Full Movement with Gains:
```{r fig.width=16, fig.height=10, echo=TRUE, warning=FALSE, message=FALSE} 
predictData <- parData %>%
  filter(flipped.gain == "2.NonUnity" & unimodal == "Self-touch" & timing %in% c("Full Movement")) %>%
  mutate(sbjN = factor(sbjN))
...
Modelling Full **Active** Movement with Gains:
```{r fig.width=16, fig.height=10, echo=TRUE, warning=FALSE, message=FALSE} 
predictData <- parData %>%
  filter(flipped.gain == "2.NonUnity" & unimodal == "Self-touch" & timing %in% c("Full Movement") & mov == "Active") %>%
  mutate(sbjN = factor(sbjN))

# Fit the linear mixed model
model <- lmer(estError ~ PowerDiff * task * gain + (1 | sbjN), data = predictData)
...
#### %Change in Beta Power as Predictor for Signed Error


```{r fig.width=12, fig.height=6, echo=FALSE, warning=FALSE, message=FALSE}
## signed error ----
plotData <- byBaseline.parData %>% filter(unimodal == "Self-touch" & flipped.gain == "2.NonUnity" & timing == "Full Movement")

ggplot(plotData, aes(x = PowerDiff, y = estError, color = task)) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  annotate("text", x = -50, y = +Inf, label = sprintf("\u2190\u2190\u2190\nmotor-focused"), color = palette$two.color1[1], hjust = 1, vjust = "top", size = 12 / .pt) +
  annotate("text", x = +50, y = +Inf, label = sprintf("\u2192\u2192\u2192\ntactile-focused"), color = palette$two.color1[2], hjust = 0, vjust = "top", size = 12 / .pt) +
  geom_point(alpha = .05, show.legend = F) +
  geom_smooth(method = "lm", show.legend = T) +
  labs(
    title = "Predicting Judgement Error with Difference of Beta Power (Non-Unity Conditions)",
...
Modelling with Full Movement:
```{r fig.width=12, fig.height=12, echo=FALSE, warning=FALSE, message=FALSE}
options(width = 500, height = 10)
predictData <- parData %>%
  filter(unimodal == "Self-touch" & timing %in% c("Full Movement") & flipped.gain == "2.NonUnity") %>%
  mutate(sbjN = factor(sbjN))
