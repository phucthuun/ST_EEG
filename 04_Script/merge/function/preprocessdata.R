...
preprocess_beh_df <- function(df) {
  ...
  df$mov <- factor(df$mov, levels = c("p", "a"), labels = c("Passive", "Active")) 
  ...
  df$flipped.gain <- factor(df$flipped.gain, labels = c("Target < Interference", "1.Unity", "Target > Interference"))
