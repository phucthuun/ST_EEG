...
## Pool data
```{r}
all_files <- list.files(path = pathlist$beh_data_path, pattern = "*.csv")
...
for (file in specific_files) {
  temp_df <- read.csv(file.path(pathlist$beh_data_path, file), header = TRUE)
  df_list[[length(df_list) + 1]] <- temp_df[, c(1:23)]
}

combined_df <- do.call(rbind, df_list) %>%   filter(NAME != "NAME")
# combined_df <- combined_df %>%
#     # Assign BLOCK number based on TIMESTAMP within each group
#     group_by(NAME, QUESTION, ACTIVE) %>%
#     mutate(
#       BLOCK = as.integer(factor(TIMESTAMP, levels = unique(TIMESTAMP))),
#       .before = GAIN
#     ) %>%
#     ungroup() %>%
#   filter(!(NAME == "P03"  & QUESTION == "t" & ACTIVE == "p" & BLOCK %in% c(3))) %>%
#   filter(!(NAME == "P17"  & QUESTION == "m" & ACTIVE == "a" & BLOCK %in% c(3))) %>%
#   filter(!(NAME == "P22"  & QUESTION == "t" & ACTIVE == "a" & BLOCK %in% c(3))) %>%
#   filter(!(NAME == "P23"  & QUESTION == "m" & ACTIVE == "p" & BLOCK %in% c(3) & TRIALNB %in% c(13))) %>%
#   # temporary filter due to unsatisfactory preprocessing
#   filter(!(NAME %in% c("P11", "P17", "P22")))

testmerge2 <- combined_df %>%
  group_by(NAME, QUESTION, ACTIVE) %>%
  summarise(n = n())
...
test <- rawData %>% # Check number of trials per blk per ppt per phase
  count(sbjN, condN)%>%
  mutate(sbjN=as.factor(sbjN))
ggplot(test) +
  geom_point(aes(x = sbjN, y = n)) +
  facet_wrap( ~ condN) +
  labs(
