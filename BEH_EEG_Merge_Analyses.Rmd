...
phases <- c("Movement Cue", "Full Movement",
                                 "Before Movement", 
                                 "During Movement", 
...
rm(list = c("beh_combined_df", "beh_test",
...
ggplot(rawData)+
  ...
  labs(x="Move duration in behavioral data (s)",
       y="Move duration in EEG data (s)")+ coord_fixed(ratio=1)

rawData %>%
  select(sbjN, condN, BLOCK, trialN, flipped.gain, Target_length, resp, 
         matches("^(C3|C4).*_(full|early|late)$")
         ) %>%
