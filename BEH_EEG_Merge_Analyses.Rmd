...
merge_df <- merge_df %>%
  filter(!(NAME == "pilot005" & unimodal == "1" & QUESTION == "m" & ACTIVE == "a" & BLOCK %in% c(1, 3))) %>%
  ...
  filter(!(NAME == "pilot007" & is.na(unimodal) & QUESTION == "t" & ACTIVE == "p" & BLOCK %in% c(2, 3))) %>%
  filter(!(NAME == "pilot008" & is.na(unimodal) & QUESTION == "m" & ACTIVE == "p" & BLOCK %in% c(1))) %>%
  filter(!(NAME == "pilot005" & unimodal == "1" & QUESTION == "m" & ACTIVE == "p" & BLOCK %in% c(1)))
```
...
rawData %>%
  select(sbjN, condN, BLOCK, trialN, flipped.gain, Target_length, resp, C3_early, C4_early, C3_late, C4_late, C3_pre, C4_pre, C3_post, C4_post) %>%
...
# EEG ----
plotData = longData %>% 
  filter(timing %in% c("Before Movement","During Movement", "Early Movement", "Late Movement", "After Movement") & 
...
byBaseline.shortData = shortData %>%
  ...
  rowwise() %>%
  # mutate(C3_dur = 100*(C3_dur - C3_pre)/ C3_pre,
  #        C4_dur = 100*(C4_dur - C4_pre)/ C4_pre,
...
plotData = byBaseline.longData %>% 
  filter(timing %in% c("Before Movement","During Movement", "Early Movement", "Late Movement", "After Movement") & unimodal == "Unimodal" & !is.na(relativehemisphere) & location %in% c("C3","C4")) %>%
  group_by(timing, condN, relativehemisphere, location) %>%
  get_summary_stats(powerBeta, show = c("ci", "mean", "sd"))
...
plotData <- longData %>%
  filter(timing == "During Movement" & unimodal == "Unimodal" & hemisphere != "mid" & location %in% c("C3", "C4"))
...
```{r fig.width=15, fig.height=8, echo=FALSE, warning=FALSE, message=FALSE}
plotData = longData %>% 
  filter(timing %in% c("Before Movement","During Movement", "After Movement") & unimodal == "Self-touch" & !is.na(relativehemisphere)) %>%
...
plotData = byBaseline.longData %>% 
  filter(timing %in% c("Before Movement","During Movement", "After Movement") & unimodal == "Self-touch" & !is.na(relativehemisphere) & location %in% c("C3","C4")) %>%
  group_by(timing, condN, relativehemisphere, location) %>%
  get_summary_stats(powerBeta, show = c("ci", "mean", "sd"))
...
ggplot(plotData,
       aes(x = C3_dur-C4_dur, y = absError, color = task))+
  geom_point(alpha = .05)+
  geom_smooth(method = "lm") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~mov)+
  labs(
    title = "Difference of Beta Power During Movement",
...
<center style="color:#15925E;">**Pre-Movement or During-Movement Difference?**</center>
...
ggplot(plotData,
  ...
  facet_wrap(~mov)+
  labs(
    title = "Difference of Beta Power Before Movement",
    x = "Beta Power Difference: C3 - C4",
    y = "Perceptual Interference \n(Normalized Absolute Error)"
  ) +
  scale_color_manual("Task", values = palette$two.color1) +
...
<p style="text-align: justify;">
**Q7: (Self-touch conditions) Does the sum in beta power in channels C3 and C4 during movement explain the movement and tactile spatial percepts regardless of task and movement type?**
...
ggplot(plotData,
  ...
  facet_wrap(~mov)+
  labs(
    title = "Sum in Beta Power During Movement",
